...
export function WorkoutComplete({
  ...
  return (
    <div className="animate-rise flex flex-col items-center text-center">
      <span className="border-accent-500/30 bg-accent-500/12 text-accent-400 shadow-glow inline-flex h-16 w-16 items-center justify-center rounded-2xl border">
        <Trophy className="h-7 w-7" aria-hidden="true" />
      </span>

      <p className="text-accent-400 mt-8 text-xs font-semibold tracking-[0.4em] uppercase">
        {title}
      </p>
      ...
      <p className="font-display text-chalk mt-10 text-7xl tabular-nums sm:text-8xl">
        {formatClock(elapsedSeconds)}
      </p>
      <p className="text-fog mt-2 text-[11px] font-semibold tracking-[0.24em] uppercase">
        Total Time
      </p>

      <dl className="border-chalk/8 bg-ink-850 divide-chalk/8 mt-12 grid w-full max-w-xl grid-cols-3 divide-x rounded-3xl border">
        {stats.map((stat) => (
          <div key={stat.label} className="px-3 py-7">

...
/** The member's finished sessions, newest first — used by workout history. */
export async function getCompletedSessions(
  limit = 5,
): Promise<{ data: CompletedSessionSummary[]; error: boolean }> {
  const user = await getSessionUser();
  ...
  const { data, error } = await supabase
    .from("workout_sessions")
    .select("id, workout_slug, completed_at, duration_seconds, total_volume, exercise_logs(count)")
    .eq("user_id", user.id)
    .eq("completed", true)
  ...
  return {
    data: data
      ...
      .map((row) => ({
        ...
        setCount:
          (row.exercise_logs as unknown as { count: number }[] | null)?.[0]?.count ?? 0,
      })),
    error: false,
  };
}
